...
# 建立语料库
def build_corpus(data_set, label_set, tokens_only=False):
    print(len(data_set))
    for i, line in enumerate(data_set):
        ...
        if tokens_only:
            ...
            yield gensim.models.doc2vec.TaggedDocument(sentence, [label_set[i]])

# 建立训练集、验证集、测试集的语料库
train_corpus = list(build_corpus(rd.train_data, rd.train_labels))
valid_corpus = list(build_corpus(rd.valid_data, rd.valid_labels, True))
test_corpus = list(build_corpus(rd.test_data, rd.valid_labels, True))

# doc2vec训练
model = gensim.models.doc2vec.Doc2Vec(vector_size=100, min_count=2, epochs = 40,workers = 4, window = 2)
...
# 保存numpy数组
np.save("code/classify/train_vectors.npy", np.array(train_vectors))
np.save("code/classify/valid_vectors.npy", np.array(valid_vectors))
np.save("code/classify/test_vectors.npy", np.array(test_vectors))
