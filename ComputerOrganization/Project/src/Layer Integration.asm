...
for5:	mul	$s6,$t0,$t0
	...
	jal	popcount		# 调用popcount函数，计算1的个数
...
print2:	mul	$t3,$s0,$t2	
	...
	addi	$s2,$0,1		# 以下注释是层整合的公式
	beq	$a0,$s7,assign		# x4=1 if x1≥ξ and γ>0
	sgt	$s4,$a0,$s7		# x4=0 if x1<ξ  and γ>0
	sgt	$s5,$s3,$0		# x4=1 if x1≤ξ and γ<0
	seq	$s2,$s4,$s5		# x4=0 if x1>ξ  and γ<0					
assign:	move	$a0,$s2								
	li    	$v0,1
        syscall				# 输出arr3对应值
        la    	$a0,space
...
popcount:				# popcount函数，用于计算一个二进制数中1的个数
	move	$t4,$0			
