...
PATH = 'tfidf/'
train_vectors = torch.from_numpy(np.load(PATH + 'train_vectors.npy'))
valid_vectors = torch.from_numpy(np.load(PATH +'valid_vectors.npy'))
test_vectors = torch.from_numpy(np.load(PATH +'test_vectors.npy'))
train_labels = torch.from_numpy(np.load(PATH +'train_labels.npy'))
valid_labels = torch.from_numpy(np.load(PATH +'valid_labels.npy'))
test_labels = torch.from_numpy(np.load(PATH +'test_labels.npy'))
...
class RNN(nn.Module):
    ...
    def forward(self, x):
        x = x.view(len(x), 1, -1)   # reshape x to (batch, time_step, input_size)
        r, _ = self.rnn(x)   
        r = self.dropout(r)
        out = self.out(r[:, -1, :])
        return out
    ...
    for step, (b_x, b_y) in enumerate(train_loader):    # gives batch data
        ...
        if step % 118 == 0:
            ...
            if valid_acc > best_acc:
                best_acc = valid_acc
                torch.save(rnn.state_dict(), 'RNN1.ckpt')
...
# test
rnn.load_state_dict(torch.load('RNN1.ckpt'))
rnn = rnn.to(device)
rnn.eval()
with torch.no_grad():
    test_x = test_vectors.to(device)
