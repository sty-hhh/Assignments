...
@app.route('/singer/edit/<int:user_id>', methods=['POST'])
...
def searchkeyword():
    ...
    results2 = Singer.query.filter(
        Singer.singer_name.like("%"+keyword+"%")).all()
    results3 = Album.query.filter(Album.album_name.like("%"+keyword+"%")).all()
    results4 = User.query.filter(User.user_name.like("%"+keyword+"%")).all()
    return render_template('keyword.html', musics=results1, singers=results2, albums=results3, users=results4,cur_user = cur_user)
...
def AlbumPage():
    ...
    paginate = Album.query.order_by(
        Album.album_name).paginate(page=page, per_page=8)
